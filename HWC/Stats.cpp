...
void Stats::reset_window() {
	...
	cout << setw(10) << new_stat.op_count_total << ", " << setw(10) << new_stat.last_timestamp << ", "
			<< setw(10) << new_stat.last_density << ", " << setw(10) << new_stat.last_size << ", "
			<< setw(10) << new_stat.max_approx_error << ", " << setw(10) << new_stat.operation_num
			<< ", " << setw(10) << new_stat.micros << ", " << setw(10)<< new_stat.micros_per_op << ", "
			<< setw(10)<< new_stat.add_op_in_window << ", " << setw(10)<< new_stat.rem_op_in_window
			<< endl;
...
void Stats::exec_op(bool is_add, bool is_report, unsigned int size_ds, double density,
		double upperbound, unsigned int last_timestamp, std::string label) {
	if (op_count_ == 0) {
		last_time_ = std::chrono::system_clock::now();
		cout << "label" << "," << "op_count_total" << ", " << "last_timestamp" << ", "
